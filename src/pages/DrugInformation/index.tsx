import { Group, ProColumns } from "@ant-design/pro-components";
...
const NormalGenes = () => {
  ...
  const uniqueGeneOptions = [...new Set(data.map((item) => item.gene))].map(
    (gene) => {
  ...
  const uniqueDrugOptions = [...new Set(data.map((item) => item.drug))].map(
    (drug) => {
