...
export default ({ record, isAddRow = true }) => {
    ...
    return <>
        <table>
            <thead>
                <tr>
                    {record?.header?.map((title, titleId) => <th key={titleId}>{title}</th>)}
                </tr>
            </thead>
            <tbody>  {record?.body?.map((res, resId) => {
                return (
                    <tr key={resId}>
                        {res.map((ques, quesId) => {
                            return (
        ...
        {
            isAddRow && <Button icon={<PlusOutlined />} className='mt-4' onClick={handleAddRow}>Thêm hàng</Button>
