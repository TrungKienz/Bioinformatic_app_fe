...
const TestDetail = () => {
  ...
  return (
    <Descriptions title="Thông tin trong báo cáo" bordered style={{padding: '20px'}}>
      ...
      <Descriptions.Item label="RunID" span={2}>{dataTest[0]?.runID}</Descriptions.Item>
      ...
      <Descriptions.Item label="Mode chiều dài đoạn đọc">{dataTest[0]?.modeLength}</Descriptions.Item>
      <Descriptions.Item label="Số đoạn đọc có thể sử dụng">{dataTest[0]?.usableReads}</Descriptions.Item>
      <Descriptions.Item label="Chiều dài trung bình đoạn đọc" >{dataTest[0]?.meanLength}</Descriptions.Item>
    </Descriptions>
  );
}
